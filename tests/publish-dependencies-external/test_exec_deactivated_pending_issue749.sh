...
# ]
#
# This output shows that the dependency for AES is satisfied by the configured
# certificate reference of C123. The dependency for the DRBG is satisifed by
# the configured certificate "DRBG 456".
#
generate_expected()
{
	local testtype=$1
...
#	result=$($EXEC -s ${testtype}/secure-datastore/ -b ${testtype}/testvectors/ -d ${testtype}/acvpproxy_0.5/ --match-expected "${EXPECTEDDIR}/${testtype}${EXPECTEDRES}" --match-actual "${ACTUALDIR}/${testtype}${ACTUALRES}")
	...
	if [ $? -ne 0 ]
	then
		echo_fail "Publish external dependencies $testtype: $result"
	else
		echo_pass "Publish external dependencies $testtype"
	fi
